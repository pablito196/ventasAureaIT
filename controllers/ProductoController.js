...
export default {
    ...
    list: async (req,res,next) =>{
        try {
            ...
            const reg = await models.Producto.find({$or:[{'codigo':new RegExp(valor,'i')},{'descripcion':new RegExp(valor,'i')}]});
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message:'Ocurrio un error'
    ...
    update: async (req,res,next) =>{
        try {
            const reg = await models.Producto.findByIdAndUpdate({_id:req.body._id},{descripcion:req.body.descripcion,codigo:req.body.codigo,
                                                                precioVenta:req.body.precioVenta,imagen:req.body.imagen});
            res.status(200).json(reg);
        } catch (e) {
            res.status(500).send({
                message:'Ocurrio un error'
